...
#Returns an array of the same shape as the input arr1
#Zero padding arr1 and calculate the sum of the elements
#of the 2D array from the convolution term
def conv(arr1, arr2):
    ...
    conv_arr = np.zeros((col_arr1, row_arr1))
    for i in range(col_arr1):
        for j in range(row_arr1):
            conv_arr[i, j] = np.sum(( arr1[i:i+col_arr2, j:j+row_arr2] * arr2 ))

    return conv_arr


#Returns an array of the same shape as the zero padded input arr1
#Follows basic concept of convolution
def basicConv(arr1, arr2):
    col_arr1 = arr1.shape[0]
    row_arr1 = arr1.shape[1]
    col_arr2 = arr2.shape[0] #should be odd number
    row_arr2 = arr2.shape[1] #should be odd number
    ...
    for i in range(col_arr1):
        for j in range(row_arr1):
            conv_arr[i:i+col_arr2, j:j+row_arr2] = conv_arr[i:i+col_arr2, j:j+row_arr2] + ( arr1[i+int((col_arr2-1)/2), j+int((row_arr2-1)/2)] * arr2 )
